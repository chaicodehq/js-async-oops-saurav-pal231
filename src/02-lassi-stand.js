 ...
 *   - this._nextOrderId = 1 (internal counter for au to-increment)
 *
...
LassiStand.prototype.addFlavor = function(flavor, price) {
  ...
  if (price <= 0 || Number.isNaN(price) || typeof price !== "number") return -1;
...
LassiStand.prototype.takeOrder = function(customerName, flavor, quantity) {
  ...
  const order = { id: this._nextOrderId++, customer: customerName, flavor, quantity, total: this.menu.find(item => item.flavor === flavor).price * quantity, status: "pending" };
...
LassiStand.prototype.getRevenue = function() { return this.orders.filter(order => order.status === "completed").reduce((total, order) => total + order.total, 0); };
LassiStand.prototype.getMenu = function() { return [...this.menu]; };
